    ...
    useEffect(() => {
        const fetchNewsData = async () => {
            try {
                setLoading(true);
                setError(null);
                const newsItems = await sapODataService.fetchNewsFeed();
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex flex-wrap items-center gap-4 text-sm">
                                    <span
                                        className={`rounded-full px-3 py-1 font-medium ${categories.find((c: Category) => c.id === activeCategory)?.color ||
                                            'text-[#83bd01]'
                                ...
                                <h1
                                    className={`text-3xl leading-tight font-bold ${categories.find((c: Category) => c.id === activeCategory)?.color ||
                                        'text-[#83bd01]'
                                        }`}
                                >
                                    {selectedNews.title}
    ...
    return (
        ...
        <div className="ml-[11px] h-px flex-grow bg-gradient-to-r from-[#83bd01] to-transparent"></div>
                                        ...
                                        <React.Fragment key={article.id}>
                                            ...
                                            >
                                                <div className="text-xl leading-5 font-normal tracking-[-0.40px] text-transparent">
                                                    <span
                                                        className={`font-bold ${categories.find((c: Category) => c.id === activeCategory)?.color ||
                                                            'text-[#83bd01]'
            ...
            <div className="text-center text-sm text-white/70">
                ...
                <span
                    className={`font-bold ${categories.find((c: Category) => c.id === activeCategory)?.color || 'text-[#83bd01]'
                        }`}
                >
                    {categories.find((c: Category) => c.id === activeCategory)?.label}
                </span>{' '}
