...
export const DashboardSection: React.FC<ExtendedDashboardSectionProps> = ({
  ...
  const orderedRenderableWidgets = orderedAllWidgets.filter((widget: any) =>
    visibleByPreference(widget.id)
  );
  const announcementWidgets = orderedRenderableWidgets.filter(
    (w: any) => w.name === 'announcement'
  );
      ...
      return (
        ...
        >
          <div className="p-4 text-center text-white">
            ...
            <p className="mt-2 text-xs">Available types: {Object.keys(widgetMapping).join(', ')}</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <div
        ...
        className={getWidgetClasses(
          ...
          `${
            isFilterPanel
              ? 'relative overflow-hidden rounded-xl transition-shadow duration-200 hover:cursor-pointer'
      ...
      >
        ...
        {isUnauthorized ? (
          <div
            className="flex h-full w-full flex-col rounded-xl bg-[var(--widget-bg)] p-4"
          ...
          {announcementWidgets.map((widget) => {
            ...
            return (
              <div
                ...
                className={getWidgetClasses(
                  widget.id,
                  `relative mb-2 rounded-xl overflow-hidden${isTransparentWidget ? 'transparent' : ''}`
                )}
                style={
                  !isTransparentWidget
                    ? { background: 'var(--widget-bg)', boxShadow: 'var(--widget-shadow)' }
