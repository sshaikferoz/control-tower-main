...
const RadarChartComponent = ({
  ...
  // Sample data for demonstration - showing skill levels or metrics
  const sampleData = [
    ...
    <div className="h-full w-full">
      <div className="h-full rounded-xl p-4 text-white" style={backgroundStyle}>
        ...
        <div className="flex h-full flex-col items-center">
          {/* Chart */}
          <div className="mt-4 h-3/4 w-full max-w-xl">
              ...
              <RadarChart data={displayData}>
                ...
                {displaySeries.map((item, index) => (
                  <Radar
                    ...
                    stroke={item.color}
                    fill={item.color}
                    fillOpacity={item.name.toLowerCase().includes('predict') ? 0.3 : 0.6}
                    strokeWidth={2}
                  />
                ))}
              </RadarChart>
          ...
          {/* Legend */}
          <div className="mx-auto mt-2 flex flex-wrap items-center justify-center gap-4">
            {displaySeries.map((item, index) => (
