...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [helpConfigStatus, setHelpConfigStatus] = useState<{
    type: 'success' | 'error';
    text: string;
  } | null>(null);
  ...
  const handleSaveHelpConfig = async () => {
    try {
      ...
      setHelpConfigStatus({
        type: 'error',
        text: 'Unable to save help information. Please try again.',
      });
    } finally {
  ...
  const handleCopyUrl = async (item: MenuItem) => {
    try {
      const baseUrl = process.env.NEXT_PUBLIC_BSP_NAME;
  ...
  // Build an absolute tab link: location origin + BSP base path + params.
  // NEXT_PUBLIC_BSP_NAME is a base path (e.g. "/sap/bc/.../app"), so it must be
  // prefixed with the origin. If it is already an absolute URL, use it as-is.
  const buildTabLink = (item: MenuItem) => {
    const origin = typeof window !== 'undefined' ? window.location.origin : '';
    const basePath = process.env.NEXT_PUBLIC_BSP_NAME || '';
    const base = /^https?:\/\//i.test(basePath) ? basePath : `${origin}${basePath}`;
    return `${base}/?view=edit&tabId=${item.id}`;
  };

  const handleCopyTabLink = async (item: MenuItem) => {
    const tabUrl = buildTabLink(item);
    try {
  ...
  const handleSaveItem = async (item: MenuItem) => {
    try {
      ...
      if (sidebarModal.mode === 'add') {
        ...
        const maxOrder =
          menuItems.length > 0 ? Math.max(...menuItems.map((i) => i.order || 0)) : -1;
    ...
    if (editMode) {
      try {
        ...
        const hasOrderChange =
          ...
          tempItems.some((item, index) => menuItems[index]?.id !== item.id);
        ...
        if (hasOrderChange) {
          const sortOrderPayload = itemsForSave
            .map((item, index) => (item.isNew ? null : { Id: item.id, SortOrd: index.toString() }))
            .filter((x): x is { Id: string; SortOrd: string } => x !== null);
          if (sortOrderPayload.length > 0) {
            await sapODataService.updateMenuItemsSortOrder(sortOrderPayload);
          }
        }
  ...
  return (
    <div
      className={`flex h-screen flex-col p-4 text-white ease-in-out ${
    ...
    >
      ...
      <nav className="mt-2 min-h-0 flex-grow overflow-y-auto">
        {isLoading ? (
                          ...
                          {editMode && !isCollapsed && (
                            <div className="flex space-x-1">
                              <button
                                ...
                                title={
                                  copiedTabLinkId === item.id ? 'Link Copied!' : 'Copy Direct Link'
                                }
                              >
                                {copiedTabLinkId === item.id ? (
                                  <CheckIcon className="h-3 w-3 text-white" />
      ...
      {!isCollapsed && (
        <div className="mt-auto flex flex-col gap-3 border-t border-white/10 pt-4">
          ...
          <div className="relative w-full" ref={helpPanelRef}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-[var(--sidebar-text)] opacity-80">
                Help
              </span>
            ...
            <div className="space-y-3">
              ...
              <div>
                <p className="text-[11px] opacity-75">
                  Paste Markdown here. Links format: [label](https://example.com) or
