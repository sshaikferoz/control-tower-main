...
const SidebarMapping: React.FC<SidebarMappingProps> = ({ onItemClick }) => {
    ...
    return (
        <div className="flex h-screen w-56 min-w-56 max-w-56 flex-shrink-0 flex-col bg-gradient-to-b from-[#00214E] to-[#0164B0] p-4 text-white md:w-64 md:min-w-64 md:max-w-64">
            ...
            <nav className="mt-6 min-h-0 flex-1">
                <ul className="h-full space-y-2 overflow-x-hidden overflow-y-auto">
                    {filteredMenuItems.map((item, index) => (
                        <li
                            key={index}
                            className={`flex-shrink-0 cursor-pointer rounded px-4 py-2 transition-colors duration-200 ${selectedItem === item.name
                                ? 'bg-white text-black'
                                ...
                                <div className="mt-1 flex min-h-[2.5rem] items-center justify-center px-1 text-center text-sm capitalize">
                                    <span className="break-words hyphens-auto">
                                        {item.name === 'pie-chart-total'
                                            ? 'Donut Chart' : item.name === 'multi-chart-bex'
                                                ? 'Multi Chart'
                                                : item.name === 'kpi-chart'
                                                    ? 'Single KPI'
